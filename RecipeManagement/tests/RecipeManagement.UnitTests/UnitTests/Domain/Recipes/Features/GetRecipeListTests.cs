...
public class GetRecipeListTests
{
    ...
    [Test]
    public async Task can_filter_recipe_list_using_HaveMadeItMyself()
    {
        //Arrange
        var fakeRecipeOne = FakeRecipe.Generate(new FakeRecipeForCreationDto()
            .RuleFor(r => r.HaveMadeItMyself, _ => false)
            .Generate());
        var fakeRecipeTwo = FakeRecipe.Generate(new FakeRecipeForCreationDto()
            .RuleFor(r => r.HaveMadeItMyself, _ => true)
            .Generate());
        var queryParameters = new RecipeParametersDto() { Filters = $"HaveMadeItMyself == {fakeRecipeTwo.HaveMadeItMyself}" };
